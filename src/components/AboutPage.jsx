...
export default function AboutPage({ onNavigate, pageData = null }) {
  ...
  const heroBlock = useMemo(
    () =>
      Array.isArray(pageData?.content)
        ? pageData.content.find((b) => b?.type === "hero")
        : null,
    [pageData],
  );
  const featuresBlocks = useMemo(
    () =>
      Array.isArray(pageData?.content)
        ? pageData.content.filter((b) => b?.type === "features")
        : [],
    [pageData],
  );
  const galleryBlock = useMemo(
    () =>
      Array.isArray(pageData?.content)
        ? pageData.content.find((b) => b?.type === "gallery")
        : null,
    [pageData],
  );
    ...
    () =>
      Array.isArray(pageData?.content)
        ? pageData.content
            .slice()
            .reverse()
            .find((b) => b?.type === "hero" && b?.data?.cta_buttons?.length)
        : null,
    [pageData],
  );
  ...
  const aboutStats =
    heroBlock?.data && Array.isArray(heroBlock.data.stats)
      ? heroBlock.data.stats.map((s) => ({ value: s.value, label: s.label }))
      : defaultStats;

  // Timeline from first features block
  const storyTimeline =
    featuresBlocks[0]?.data && Array.isArray(featuresBlocks[0].data.items)
      ? featuresBlocks[0].data.items.map((item) => ({
          year:
            Array.isArray(item.labels) && item.labels.length > 1
              ? item.labels[1]
              : "",
          title: item.title || "",
  ...
  const coreValues =
    featuresBlocks[1]?.data && Array.isArray(featuresBlocks[1].data.items)
  ...
  // Gallery
  const galleryItems =
    galleryBlock?.data && Array.isArray(galleryBlock.data.images)
  ...
  return (
    <main className="about-page page-shell">
      ...
      <section className="about-hero motion-section">
        ...
        <div
          className="about-hero-lottie motion-item"
          style={{
            display: "flex",
      ...
      <section className="about-story section-shell motion-section">
        <div className="section-heading">
          <h2 className="motion-item">
            {featuresBlocks[0]?.data?.title || "Perjalanan & Cerita Kami"}
          </h2>
          <p className="motion-item">
            {featuresBlocks[0]?.data?.subtitle ||
      ...
      <section className="about-values section-shell motion-section">
        <div className="section-heading">
          <h2 className="motion-item">
            {featuresBlocks[1]?.data?.title ||
              "Nilai Inti & Pilar Sirkularitas"}
          </h2>
          <p className="motion-item">
            {featuresBlocks[1]?.data?.subtitle ||
      ...
      <ErrorBoundary3D
        fallback={
          <section
            className="team-lanyards-container section-shell"
            aria-hidden="true"
          />
        }
      >
      ...
      <section className="about-gallery section-shell motion-section">
        <div className="section-heading">
          <h2 className="motion-item">
            {galleryBlock?.data?.title || "Infrastruktur & Di Balik Layar"}
          </h2>
          <p className="motion-item">
            {galleryBlock?.data?.subtitle ||
        ...
        <div className="gallery-bento">
          {galleryItems.map(({ title, category, image }, index) => (
            <article className="gallery-item motion-item" key={index}>
              {image && (
                <img
                  src={image}
                  alt={title}
      ...
      <section className="about-cta section-shell motion-section">
        <div className="cta-box motion-item">
          <div className="cta-content">
            ...
            <h2>
              {ctaData.headline || "Mari Bangun Masa Depan Pangan Bersama Kami"}
            </h2>
            <p>
              {ctaData.sub_headline ||
