...
export default function ArticleCatalog({ posts = [], categories = [], onNavigate }) {
  ...
  const articleCategories = useMemo(() => {
    ...
    allArticles.forEach((post) => {
      if (post.category_name && post.category_name !== 'Artikel' && post.category_name !== 'Produk') {
        cats.add(post.category_name)
      } else if (post.category_slug && post.category_slug !== 'artikel' && post.category_slug !== 'produk') {
  ...
  const filteredArticles = useMemo(() => {
    ...
    return allArticles.filter((post) => {
      const categoryMatch = filter === 'Semua' ||
        String(post?.category_name || post?.category_slug || '').toLowerCase() === filter.toLowerCase()
      const title = String(post?.title || '').toLowerCase()
      const excerpt = String(post?.excerpt || postContent(post).excerpt || '').toLowerCase()
      const queryMatch = !normalized || title.includes(normalized) || excerpt.includes(normalized)
    ...
    <main className="catalog article-catalog page-shell" id="katalog-artikel">
      <header className="catalog-hero motion-section">
        <div>
          ...
          <h1 className="motion-item">Kabar & Artikel <strong>JagoFarm</strong></h1>
      ...
      <div className="catalog-toolbar motion-section">
        <h2 className="motion-item">Daftar Artikel ({filteredArticles.length})</h2>
        ...
        {filteredArticles.map((post) => {
          ...
          return (
            <article className="article-card" key={post.id || post.slug}>
              <button
                type="button"
                onClick={() => onNavigate?.(`/artikel/${encodeURIComponent(post.slug)}`)}
              >
                <div className="article-card-media">
                  {img && <img src={img} alt={post.title || ''} referrerPolicy="no-referrer" loading="lazy" />}
                </div>
                <div className="article-card-copy">
                  <small>{formatCmsDate(post.published_at || post.created_at)}</small>
