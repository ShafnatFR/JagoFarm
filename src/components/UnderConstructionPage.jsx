...
export default function UnderConstructionPage({
    ...
    switch (targetPage) {
      case "home":
        return {
          ...
          description:
            "Tim teknis kami saat ini sedang melakukan pembaruan menyeluruh langsung pada struktur halaman Beranda JagoFarm. Seluruh siklus circular economy, pemantauan data sensor IoT, dan sajian visualisasi interaktif sedang disempurnakan demi menghadirkan standar masa depan pangan terintegrasi yang lebih canggih.",
      ...
      case "catalog":
        return {
          ...
          description:
            "Halaman Katalog Produk JagoFarm (mulai dari ayam probiotik super, telur omega presisi, gurame & lele sirkular, hingga pupuk cair organik dan maggot BSF) saat ini sedang diperbarui dengan kurasi dan spesifikasi terbaru langsung dari pusat farm kami.",
      ...
      case "about":
        return {
          ...
          description:
            "Kisah perjalanan JagoFarm, filosofi nol limbah (Zero Waste), serta pengenalan tim rekayasawan dan praktisi agronomi kami saat ini sedang dalam proses redaksi untuk menyajikan transparansi dan kolaborasi yang lebih mendalam.",
      ...
      case "contact":
        return {
          ...
          description:
            "Kami sedang melakukan pemeliharaan pada sistem formulir otomatis dan integrasi komunikasi tim JagoFarm. Sementara penyesuaian dilakukan, Anda tetap dapat terhubung melalui tautan alternatif yang aktif di bawah ini.",
      ...
      default:
        return {
          ...
          description:
            "Halaman yang Anda tuju saat ini dinonaktifkan sementara dari panel administrator CMS untuk proses pembaruan sistem, integrasi API terbaru, dan penyempurnaan antarmuka.",
  ...
  if (
    !cmsLoaded ||
    (Array.isArray(pages) &&
      pages.some((p) => p?.slug === "beranda" || p?.slug === "home"))
  ) {
    if (page !== "home")
      activeLinks.push({
        label: "Beranda Utama",
        target: "#beranda",
        desc: "Kembali ke halaman utama ekosistem JagoFarm",
      });
  }
  if (
    !cmsLoaded ||
    (Array.isArray(pages) &&
      pages.some((p) => p?.slug === "produk" || p?.slug === "katalog"))
  ) {
    if (page !== "catalog")
      activeLinks.push({
        label: "Katalog Produk",
        target: "/produk",
        desc: "Jelajahi hasil panen organik & teknologi sirkular",
      });
  }
  if (
    !cmsLoaded ||
    (Array.isArray(pages) &&
      pages.some((p) => p?.slug === "tentang-kami" || p?.slug === "about"))
  ) {
    if (page !== "about")
      activeLinks.push({
        label: "Tentang Kami",
        target: "/tentang-kami",
        desc: "Kenali visi, misi, dan jajaran tim pemikir JagoFarm",
      });
  }
  if (
    !cmsLoaded ||
    (Array.isArray(pages) &&
      pages.some((p) => p?.slug === "hubungi-kami" || p?.slug === "kontak"))
  ) {
    if (page !== "contact")
      activeLinks.push({
        label: "Hubungi Kami",
        target: "/hubungi-kami",
        desc: "Konsultasi kolaborasi, kunjungan farm, & layanan cepat",
      });
  }
  ...
  return (
    <main
      ...
      style={{
        ...
        alignItems: "center",
        padding:
          "clamp(140px, 14vh, 180px) clamp(24px, 6vw, 96px) clamp(80px, 10vh, 140px)",
        overflow: "hidden",
      }}
    >
          ...
          >
            ...
            {activeLinks.length > 0 ? (
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      ...
                      e.currentTarget.style.boxShadow =
                        "0 16px 40px rgba(12, 94, 54, 0.12)";
              ...
              >
                ...
                >
                  Seluruh halaman sedang dalam pemeliharaan sistem JagoFarm.
                </p>
              </div>
            )}
          </div>
        ...
        >
          ...
          <div
            style={{
              ...
              borderRadius: "50%",
              background:
                "radial-gradient(circle, var(--leaf-soft) 0%, transparent 70%)",
              filter: "blur(36px)",
              zIndex: 0,
            }}
          />
